...
public static class SerilogConfiguration
{
    ...
    private static readonly string[] GatewayRoutineMessages =
    [
        // "Server requested a reconnect", // message generated by me based on exception log (technically unnecessary)
        "Disconnecting",
        "Disconnected",
        "Connecting",
        "Connected",
    ...
    private static bool IsGatewayRoutineMessage(LogEvent e)
    {
        if (!e.Properties.TryGetValue("Source", out var source) || source.ToString() != "\"Gateway\"")
            return false;
    
        if (!e.Properties.TryGetValue("Message", out var message))
            return false;
    
        return GatewayRoutineMessages.Contains(message.ToString().Trim('"'));
    }
    ...
    public static void Configure(HostBuilderContext ctx, LoggerConfiguration log)
    {
        ...
        log
            .MinimumLevel.Debug()
            .Filter.ByExcluding(IsGatewayRoutineMessage)
